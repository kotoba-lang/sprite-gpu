(ns kotoba.sprite-gpu
  "2D as GPU instanced quads — the bridge that moves the sprite renderer off Canvas2D and onto the
   GPU pipeline IR (kotoba.gpu). Every sprite primitive (circle / ellipse / rect / arc) becomes one
...
(defn anim-quad
  "Apply a primitive's data-declared :anim (sprite2d vocabulary — {:rot :pulse :bob :sway [amp freq]})
   to its quad at time `t` with per-entity phase `ph`, mirroring kotoba.sprite2d's tick-driven motion:
...
(defn draw-ops->quads
  "Flatten a kotoba.sprite2d.layout draw-list (ops {:sprite [prims…] :sx :sy :ph?}) into one flat quad
...
;; ── the 2D-SDF sprite shader (instanced quads), generated from kotoba.wgsl EDN ───────────────────
;; one instanced quad per primitive; the fragment evaluates the shape's signed-distance field for an
;; anti-aliased fill. Pure raster (no compute/storage) → runs on WebGPU AND WebGL2 (kotoba.gpu has no
